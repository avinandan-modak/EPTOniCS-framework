"""
run_portal_frame.py — Example 3: V-notched portal (manuscript Section 4.3)
=============================================================================
Three-dimensional elastoplastic BESO topology optimization of a
simply-supported portal frame with a V-shaped lower void, using J2
...
Geometry (units: mm)
---------------------
The portal is carved from a bounding-box mesh via
``dolfinx.mesh.create_submesh``; a V-shaped lower void is removed to
produce the arch-like frame.

    Bounding box   : 120 × 60 × 20
    V-void bases   : (6, 0) and (114, 0)
    V-void apex    : (60, 40)
    Left column    : x ∈ [0, 6]
    Right column   : x ∈ [114, 120]
    Top beam       : y ∈ [40, 60]

Boundary conditions (simply-supported)
----------------------------------------
    Left support  (PIN)   : u_x = u_y = u_z = 0 on the bottom-left patch
    Right support (ROLLER): u_y = u_z = 0, u_x free, on the bottom-right patch
    Load          : u_y = Δu (downward) on the top-centre patch,
                    x ∈ [45, 75], z ∈ [0, 20]
...
Section 4.3 — Numerical example 3: V-notched portal

Usage
-----
    mpirun -np <N> python -u run_portal_frame.py

Output XDMF files are written to ``BESO_PortalFrame_Outputs/``.
"""
...
os.makedirs("BESO_PortalFrame_Outputs", exist_ok=True)   # every rank creates this; exist_ok avoids a race error

# ============================================================================
# Geometry parameters
# ============================================================================
Lx, Ly, Lz = 120.0, 60.0, 20.0   # bounding box (mm)

# V-void geometry
x_void_left  = 6.0     # left base of the V-void
x_void_right = 114.0   # right base of the V-void
x_apex       = 60.0    # x-coordinate of the V's apex
y_apex       = 40.0    # y-coordinate (height) of the V's apex

# Load patch on y = Ly (top face, centred)
load_x_lo, load_x_hi = 45.0, 75.0   # x-extent of the loaded patch
load_z_lo, load_z_hi =  0.0, 20.0   # full thickness

nx, ny, nz = 120, 60, 40   # element counts of the full bounding-box mesh

# ============================================================================
# Step 1 — Create portal-frame submesh (bounding box minus V-void)
# ============================================================================
_box = mesh.create_box(
    comm=MPI.COMM_WORLD,
    points=((0.0, 0.0, 0.0), (Lx, Ly, Lz)),
    n=(nx, ny, nz),
    cell_type=mesh.CellType.hexahedron)          # full rectangular bounding-box mesh, before removing the V-void
...
# Piecewise-linear V-void boundary
def y_void(x):
    """Height of the V-void's upper boundary at a given x (piecewise-linear, apex at x_apex)."""
    slope = y_apex / (x_apex - x_void_left)      # rise/run from the left base up to the apex
    return np.where(
        x <= x_apex,
        slope * (x - x_void_left),                # rising left half of the V
        slope * (x_void_right - x)                 # falling right half of the V (same slope, by symmetry)
    )

tol_geom = 1e-12   # tight geometric tolerance for the void-membership test

_void_mask = (
    (_centroids[:, 0] >= x_void_left  - tol_geom) &      # within the V's x-span
    (_centroids[:, 0] <= x_void_right + tol_geom) &
    (_centroids[:, 1] < y_void(_centroids[:, 0]) - tol_geom)   # below the V's upper boundary at this x
)

_portal_mask    = ~_void_mask                                        # every element NOT inside the void
_portal_indices = np.where(_portal_mask)[0].astype(np.int32)          # element indices that form the portal frame
pout(f"[BESO] Portal-frame cells: {len(_portal_indices)} / {len(_portal_mask)}", flush=True)

prob_mesh, _cell_map, _vertex_map, _geom_map = mesh.create_submesh(
    _box, _box.topology.dim, _portal_indices)     # the actual portal-frame mesh that all analysis runs on

gdim      = prob_mesh.topology.dim
dx_domain = Measure("dx", domain=prob_mesh)         # standard cell-integration measure over the portal mesh
...
# ============================================================================
# Step 5 — Boundary conditions on the portal submesh
# ============================================================================
tol = 1e-10   # geometric tolerance for facet-location comparisons

# Guard against empty local arrays on ranks with no geometry nodes
_X = prob_mesh.geometry.x
_comm = prob_mesh.comm
_INF = np.finfo(float).max
_NINF = -np.finfo(float).max
xmin = _comm.allreduce(np.min(_X[:, 0]) if len(_X) > 0 else _INF,  op=MPI.MIN)   # domain extents, safe on empty ranks
xmax = _comm.allreduce(np.max(_X[:, 0]) if len(_X) > 0 else _NINF, op=MPI.MAX)
ymin = _comm.allreduce(np.min(_X[:, 1]) if len(_X) > 0 else _INF,  op=MPI.MIN)
ymax = _comm.allreduce(np.max(_X[:, 1]) if len(_X) > 0 else _NINF, op=MPI.MAX)
zmin = _comm.allreduce(np.min(_X[:, 2]) if len(_X) > 0 else _INF,  op=MPI.MIN)
zmax = _comm.allreduce(np.max(_X[:, 2]) if len(_X) > 0 else _NINF, op=MPI.MAX)

# Left support: PIN — fix u_x, u_y, u_z
_sup_left_facets = mesh.locate_entities_boundary(
    prob_mesh, gdim-1,
    lambda x: (np.isclose(x[1], 0.0, atol=tol) &                       # bottom face (y = 0)
               (x[0] >= 0.0 - tol) & (x[0] <= x_void_left + tol)))      # left column footprint only

# Right support: ROLLER — fix u_y, u_z only
_sup_right_facets = mesh.locate_entities_boundary(
    prob_mesh, gdim-1,
    lambda x: (np.isclose(x[1], 0.0, atol=tol) &                       # bottom face (y = 0)
               (x[0] >= x_void_right - tol) & (x[0] <= Lx + tol)))      # right column footprint only

# Load patch: y = Ly, x ∈ [load_x_lo, load_x_hi], full z
_load_facets = mesh.locate_entities_boundary(
    prob_mesh, gdim-1,
    lambda x: (np.isclose(x[1], Ly, atol=tol) &                        # top face (y = Ly)
               (x[0] >= load_x_lo - tol) & (x[0] <= load_x_hi + tol) &  # within the loaded x-range
               (x[2] >= load_z_lo - tol) & (x[2] <= load_z_hi + tol)))  # full through-thickness

support_left_dofs    = fem.locate_dofs_topological(V,        gdim-1, _sup_left_facets)    # all 3 components, PIN
support_right_y_dofs = fem.locate_dofs_topological(V.sub(1), gdim-1, _sup_right_facets)   # ROLLER: y fixed
support_right_z_dofs = fem.locate_dofs_topological(V.sub(2), gdim-1, _sup_right_facets)   # ROLLER: z fixed, x free
bc_load_dofs         = fem.locate_dofs_topological(V.sub(1), gdim-1, _load_facets)         # y-displacement, loaded face
...
pout(f"[BESO] Left support facets  : {len(_sup_left_facets)}", flush=True)
pout(f"[BESO] Right support facets : {len(_sup_right_facets)}", flush=True)
pout(f"[BESO] Load facets          : {len(_load_facets)}", flush=True)


def get_bc_incremental(disp_increment):
    """PIN left, ROLLER right, prescribed u_y load."""
    bc_left  = fem.dirichletbc(ScalarType((0., 0., 0.)), support_left_dofs, V)        # PIN: u = (0,0,0)
    bc_right_y = fem.dirichletbc(ScalarType(0.0), support_right_y_dofs, V.sub(1))     # ROLLER: u_y = 0
    bc_right_z = fem.dirichletbc(ScalarType(0.0), support_right_z_dofs, V.sub(2))     # ROLLER: u_z = 0 (u_x free)
    bc_load  = fem.dirichletbc(ScalarType(disp_increment), bc_load_dofs, V.sub(1))    # this step's incremental u_y
    return [bc_left, bc_right_y, bc_right_z, bc_load]


def get_bc_homogeneous():
    """All supports and load locked to zero (Newton corrector steps)."""
    bc_left  = fem.dirichletbc(ScalarType((0., 0., 0.)), support_left_dofs, V)
    bc_right_y = fem.dirichletbc(ScalarType(0.0), support_right_y_dofs, V.sub(1))
    bc_right_z = fem.dirichletbc(ScalarType(0.0), support_right_z_dofs, V.sub(2))
...
# ============================================================================
load_incr = -0.05 * np.ones(5)   # 5 equal displacement increments of -0.05 mm each (negative: pushing downward)
n_load    = len(load_incr)        # number of load steps per BESO design iteration

max_iter  = 130     # maximum BESO design iterations
er        = 0.02    # evolutionary ratio, fraction of volume removed per iteration (Eq. 47)
volfrac   = 0.15    # target final volume fraction
...
vol_dom_local = fem.assemble_scalar(
    fem.form(fem.Constant(prob_mesh, ScalarType(1.)) * dx_domain))   # this rank's volume
vol_dom = prob_mesh.comm.allreduce(vol_dom_local, op=MPI.SUM)   # total portal-domain volume, all ranks
vol_el  = vol_dom / n_ele_global                                 # volume of one (uniform) element
pout(f"[BESO] Portal-domain vol = {vol_dom:.1f}  vol_el = {vol_el:.2f}", flush=True)
...
rho_xdmf         = io.XDMFFile(prob_mesh.comm, "BESO_PortalFrame_Outputs/density.xdmf",         "w")   # density evolution
vonMises_xdmf    = io.XDMFFile(prob_mesh.comm, "BESO_PortalFrame_Outputs/vonMises_solid.xdmf",  "w")
PEEQ_xdmf        = io.XDMFFile(prob_mesh.comm, "BESO_PortalFrame_Outputs/PEEQ_solid.xdmf",      "w")
density_vis_xdmf = io.XDMFFile(prob_mesh.comm, "BESO_PortalFrame_Outputs/density_visual.xdmf",  "w")
...
for i in range(max_iter):
    ...
    for i_load in range(n_load):
        ...
        fw += MPI.COMM_WORLD.allreduce(fw_local_step, op=MPI.SUM)          # running total work objective, summed across ranks
        ...
        # Adjoint solve (★2)
        adjoint_bcs = [
            fem.dirichletbc(ScalarType((0., 0., 0.)), support_left_dofs, V),          # PIN, zero (adjoint has no support motion)
            fem.dirichletbc(ScalarType(0.0), support_right_y_dofs, V.sub(1)),         # ROLLER y, zero
            fem.dirichletbc(ScalarType(0.0), support_right_z_dofs, V.sub(2)),         # ROLLER z, zero
...
with io.XDMFFile(MPI.COMM_WORLD, "BESO_PortalFrame_Outputs/optimized_geometry.xdmf", "w") as f:
    f.write_mesh(opt_mesh)                                 # write the final optimized geometry, viewable in ParaView
pout("Optimized geometry → BESO_PortalFrame_Outputs/optimized_geometry.xdmf", flush=True)
